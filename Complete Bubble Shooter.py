    ...
    def is_stuck(self, bubbles): 
        for bubble in bubbles:
            if self.collide(bubble):
                remove = set([])
                if self.color == bubble.get_color():
                    remove.add(bubble)
     
                bubbles.difference_update(remove)    
                return True
        if self.pos[1] <= 0+BUBBLE_RADIUS:
            return True
        else:
            return False   
    def collide(self, bubble):
        distance = dist(self.get_pos(), bubble.get_pos())
...
def keydown(key):
    ...
    if simplegui.KEY_MAP["space"] == key and bubble_stuck == True:
        bubble_stuck = False
        vel = angle_to_vector(firing_angle)
        a_bubble.fire_bubble([8 * vel[0], -8 * vel[1]])
...
def draw(canvas):
    ...
    a_bubble.update()
    if a_bubble.is_stuck(stuck_bubbles) == True:
        a_bubble.fire_bubble([0, 0])
    ...
    for bubble in list(stuck_bubbles):
        bubble.draw(canvas)
